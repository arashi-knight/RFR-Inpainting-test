...
class Config():
    ...
    # 损失函数权重
    loss_hole_weight = 60.0
    loss_valid_weight = 10.0
    loss_l1_weight = 1.0
    loss_pyramid_weight = 0.5
    loss_adversarial_weight = 0.1
    loss_texture_weight = 1.0
...
def update_config(config: Config, **kwargs):
    ...
    if 'train_id' in kwargs:
        ...
        # psnr折线图保存路径
        psnr_img_save_path: str = os.path.join(config.result_path, 'psnr_img.png')
        # ssim折线图保存路径
        ssim_img_save_path: str = os.path.join(config.result_path, 'ssim_img.png')
